...
- `config/golden-corpus/beta-validation-golden-corpus-v1.json`: 베타 validation/import precision용 seed 15건, human-reviewed
- `tests/beta-validation-golden-corpus.test.ts`: 위 샘플의 expected / non-expected rule과 import 실패 분류를 CI에서 검증
...
- 14개: synthetic circuit validation 샘플
- 1개: unsupported import 실패 샘플
- 포함 영역: I2C pull-up, direct rail short, LED series resistor, HC-06 UART level shift, op-amp feedback/range, ADC source impedance, power headroom, generic/custom symbol conservative review
