...
- 피드백 폼: `docs/beta-feedback-form-template.md`의 필수 질문을 실제 폼으로 옮겨서 사용

## 테스트 스크립트

1. `.kicad_sch` 또는 `.kicad_pcb` 파일을 올린다.
2. 회로 또는 PCB가 화면에 보이는지 확인한다.
3. 큰 회로도라면 상단 툴바의 `읽기 보기`를 눌러 부품명과 배선 라벨이 읽히는지 확인한다.
4. PCB라면 검토 요약을 먼저 보고, 필요할 때 `공식/보조 묶음 보기` 또는 `상위 묶음 보기`를 펼친다.
5. validation panel에서 critical/warning/review 항목을 하나씩 클릭한다.
6. 각 경고에서 확실도, 근거 요약, 관측 사실, 확인 방법을 읽는다.
7. 실제로 해결한 항목은 `수정 완료`, 이미 설계에 반영된 항목은 `이미 반영됨`, 오탐은 `오탐 신고`로 표시한다.
8. 데이터시트로 확인한 항목은 `데이터시트 확인`을 켠다.
9. 리포트 페이지를 열고 PDF 또는 JSON으로 공유한다.
10. 실패했거나 이해가 안 된 항목은 파일 종류, 화면, 경고 제목을 피드백으로 남긴다.

## 피드백 양식

telemetry를 켜지 않는 제한 베타에서는 최소한 아래 항목을 수동으로 받는다.

실제 폼은 `docs/beta-feedback-form-template.md`를 기준으로 만든다. 아래 블록은 빠른 메모용 축약본이다.
