...
class Schematic:
    ...
    def __init__(
        ...
        # Populate the symbol library cache with the schematic's own lib_symbols
        # to ensure that rescue/custom symbols are fully available during routing and API use.
        lib_symbols = self._data.get("lib_symbols", {})
        if lib_symbols:
            cache = get_symbol_cache()
            for lib_id, symbol_sexp in lib_symbols.items():
                if hasattr(cache, "add_symbol_from_sexp"):
                    cache.add_symbol_from_sexp(lib_id, symbol_sexp)
    ...
    def _sync_components_to_data(self):
        ...
        # Populate lib_symbols, preserving existing definitions if new ones aren't found in cache
        lib_symbols = dict(self._data.get("lib_symbols", {}))
        cache = get_symbol_cache()

        for comp in self._components:
            if comp.lib_id:
                # Only overwrite/add if we actually find a definition in the cache
                ...
                # If symbol_def is None, we preserve the original definition from the file
