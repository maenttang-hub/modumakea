...
class SchematicSymbol:
    ...
    def _init_default_properties(self) -> None:
        """Initialize default S-expressions for standard properties if they are not already present."""
        if self.properties is None:
            self.properties = {}

        import sexpdata
        from .property_positioning import get_property_position
        from .config import config

        pos = self.position or Point(0, 0)
        lib_id = self.lib_id or ""
        is_power_symbol = "power:" in lib_id
        rotation = self.rotation or 0.0

        # Helper to create property dict and sexp
        def create_prop(name: str, val: str, hide: bool, offset_index: int) -> Tuple[Dict[str, Any], List[Any]]:
            try:
                prop_x, prop_y, text_rotation = get_property_position(
                    lib_id, name, (pos.x, pos.y), rotation
                )
            except Exception:
                prop_x, prop_y, text_rotation = config.get_property_position(
                    name, (pos.x, pos.y), offset_index, rotation
                )
            ...
            justify = "left"
            if not hide or name in ["Reference", "Value"]:
            ...
            prop_dict = {
                ...
                "value": val,
                "hidden": hide,
                "at": [prop_x, prop_y, text_rotation],
                "effects": {
                    "font": [sexpdata.Symbol("font"), [sexpdata.Symbol("size"), 1.27, 1.27]],
                    "justify": justify,
                    "hide": "yes" if hide else "no"
                }
            }
            return prop_dict, prop_sexp

        # 1. Reference
        if self.reference and "__sexp_Reference" not in self.properties:
            ref_hide = is_power_symbol
            p_dict, p_sexp = create_prop("Reference", self.reference, ref_hide, 0)
            self.properties["Reference"] = p_dict
            self.properties["__sexp_Reference"] = p_sexp

        # 2. Value
        if self.value and "__sexp_Value" not in self.properties:
            val_hide = is_power_symbol
            p_dict, p_sexp = create_prop("Value", self.value, val_hide, 1)
            self.properties["Value"] = p_dict
            self.properties["__sexp_Value"] = p_sexp

        # 3. Footprint
        if self.footprint is not None and "__sexp_Footprint" not in self.properties:
            fp_hide = True
            p_dict, p_sexp = create_prop("Footprint", self.footprint, fp_hide, 2)
            self.properties["Footprint"] = p_dict
            self.properties["__sexp_Footprint"] = p_sexp
    ...
    def update_position(self, new_pos: Point, new_rot: float) -> None:
        """Update symbol position and rotation, translating all property S-expressions."""
        # Calculate rotation difference
        delta_rot = new_rot - self.rotation

        # Calculate translation displacement
        dx = new_pos.x - self.position.x
        dy = new_pos.y - self.position.y

        # We need to update all properties (Reference, Value, Footprint, and custom properties)
        # in self.properties that have corresponding __sexp_ keys.
        import math

        # Convert rotation to radians
        angle_rad = math.radians(delta_rot)
        cos_a = math.cos(angle_rad)
        sin_a = math.sin(angle_rad)

        for key, prop_val in list(self.properties.items()):
            if key.startswith("__sexp_"):
                # prop_val is the S-expression list: (property name value (at x y rot) (effects ...))
                # Let's find the "at" clause
                at_clause = None
                for sub_item in prop_val:
                    if isinstance(sub_item, list) and len(sub_item) > 0 and str(sub_item[0]) == "at":
                        at_clause = sub_item
                        break

                if at_clause and len(at_clause) >= 3:
                    # Original absolute position of the property
                    x_prop = float(at_clause[1])
                    y_prop = float(at_clause[2])
                    rot_prop = float(at_clause[3]) if len(at_clause) > 3 else 0.0

                    # 1. Translate relative to original symbol center:
                    rx = x_prop - self.position.x
                    ry = y_prop - self.position.y

                    # 2. Rotate the relative offset around symbol center:
                    rx_rot = rx * cos_a - ry * sin_a
                    ry_rot = rx * sin_a + ry * cos_a

                    # 3. Translate to the new symbol center:
                    x_prop_new = new_pos.x + rx_rot
                    y_prop_new = new_pos.y + ry_rot

                    # 4. Update the rotation of the property text:
                    rot_prop_new = (rot_prop + delta_rot) % 360

                    # Round for clean S-expressions
                    at_clause[1] = round(x_prop_new, 4) if x_prop_new != int(x_prop_new) else int(x_prop_new)
                    at_clause[2] = round(y_prop_new, 4) if y_prop_new != int(y_prop_new) else int(y_prop_new)
                    if len(at_clause) > 3:
                        at_clause[3] = round(rot_prop_new, 4) if rot_prop_new != int(rot_prop_new) else int(rot_prop_new)
                    else:
                        at_clause.append(round(rot_prop_new, 4) if rot_prop_new != int(rot_prop_new) else int(rot_prop_new))

        # Finally, update symbol's own position and rotation
