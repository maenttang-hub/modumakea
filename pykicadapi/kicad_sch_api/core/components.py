...
class PropertyValue(str):
    """
    A string subclass that preserves property metadata and allows dict-like access.

    This enables backward compatibility with code that expects properties to be
    dictionaries of metadata (e.g., positioning, effects) while supporting
    standard string operations and checks.
    """
...
class ComponentProperties(dict):
    """
    A dictionary wrapper for component properties that yields PropertyValue objects
    and propagates mutations back to the underlying symbol data dictionary.
    """
    def __init__(self, data_dict, on_modified=None):
        self._data_dict = data_dict
        self._on_modified = on_modified
        super().__init__(data_dict)

    def __getitem__(self, key):
        val = self._data_dict[key]
        if isinstance(val, dict) and "value" in val:
            return PropertyValue(val["value"], val)
        return val
...
class Component:
    ...
    def update_position(self, new_pos: Point, new_rot: float) -> None:
        """Update component position and rotation, translating all property S-expressions."""
        self._data.update_position(new_pos, new_rot)
        self._collection._mark_modified()
    ...
    def properties(self) -> Dict[str, str]:
        ...
        return ComponentProperties(self._data.properties, self._collection._mark_modified)

    def get_property(self, name: str, default: Optional[str] = None) -> Optional[str]:
        """Get property value by name."""
        val = self._data.properties.get(name, default)
        if isinstance(val, dict) and "value" in val:
            return val["value"]
        return val
