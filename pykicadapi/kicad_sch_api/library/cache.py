...
class SymbolDefinition:
    ...
    def __post_init__(self):
        ...
        # Validate lib_id format - allow no-colon for local/rescue symbols
...
class SymbolLibraryCache:
    ...
    def get_symbol(self, lib_id: str) -> Optional[SymbolDefinition]:
        ...
        symbol = self._load_symbol(lib_id)
    ...
    def add_symbol_from_sexp(self, lib_id: str, symbol_sexp: List[Any]) -> bool:
        """
        Add a symbol definition directly from a parsed S-expression (e.g. from schematic's lib_symbols).
        """
        try:
            start_time = time.time()
            if not isinstance(symbol_sexp, list) or len(symbol_sexp) < 2:
                return False

            # Extract library name and symbol name from lib_id
            if ":" in lib_id:
            ...
            extends_symbol = self._check_extends_directive(symbol_sexp)

            # Extract pins and properties
            result = {
                "raw_data": symbol_sexp,
                "reference_prefix": "U",
                "description": "",
                "keywords": "",
                "datasheet": "~",
                "pins": [],
                "extends": extends_symbol,
                "property_positions": {},
            }

            # Extract properties
            for item in symbol_sexp[1:]:
                if isinstance(item, list) and len(item) > 0:
                    if item[0] == sexpdata.Symbol("property"):
                        ...
                        elif prop_name_str == "Description":
                            result["Description"] = str(prop_value)
                        elif prop_name_str == "ki_keywords":
                            result["keywords"] = str(prop_value)
                        elif prop_name_str == "Datasheet":
                            result["Datasheet"] = str(prop_value)

            # Extract pins
            result["pins"] = self._extract_pins_from_symbol(symbol_sexp)

            # Extract unit count
            result["units"] = self._count_symbol_units(symbol_sexp)

            # Create SymbolDefinition
            symbol = SymbolDefinition(
                lib_id=lib_id,
                name=symbol_name,
                library=library_name,
                reference_prefix=result.get("reference_prefix", "U"),
                description=result.get("Description", result.get("description", "")),
                keywords=result.get("keywords", ""),
                datasheet=result.get("Datasheet", result.get("datasheet", "~")),
                pins=result.get("pins", []),
                units=result.get("units", 1),
                extends=result.get("extends"),
                property_positions=result.get("property_positions", {}),
                load_time=time.time() - start_time,
            )
            symbol.raw_kicad_data = symbol_sexp

            # Add to cache dictionaries
            self._symbols[lib_id] = symbol
            self._symbol_index[symbol_name] = lib_id

            logger.info(f"Loaded symbol {lib_id} from schematic lib_symbols cache.")
            return True

        except Exception as e:
            logger.error(f"Error adding symbol {lib_id} from S-expression: {e}")
            return False
            ...
            for item in pin_data[3:]:
                if isinstance(item, list) and len(item) > 0:
                    if item[0] == sexpdata.Symbol("at"):
                        ...
                        parsed_name = str(item[1]).strip('"')
                        name = parsed_name if parsed_name != "" else "~"
