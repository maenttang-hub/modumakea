...
class SymbolParser(BaseElementParser):
        ...
        try:
            ...
            for sub_item in item[1:]:
                ...
                if element_type == "lib_id":
                    ...
                    val = sub_item[1] if len(sub_item) > 1 else None
                    symbol_data["lib_name"] = str(val) if isinstance(val, sexpdata.Symbol) else val
                elif element_type == "body_style":
                    symbol_data["body_style"] = int(sub_item[1]) if len(sub_item) > 1 else 0
                elif element_type == "mirror":
                    val = sub_item[1] if len(sub_item) > 1 else None
                    symbol_data["mirror"] = str(val) if isinstance(val, sexpdata.Symbol) else val
    ...
    def _symbol_to_sexp(self, symbol_data: Dict[str, Any], schematic_uuid: str = None) -> List[Any]:
        ...
        # Add body_style if non-zero
        body_style = symbol_data.get("body_style", 0)
        if body_style != 0:
        ...
        if in_pos_files is not None:
            sexp.append(
                [
                    sexpdata.Symbol("in_pos_files"),
                    "yes" if in_pos_files else "no",
                ]
            )
